...
public class BlockEventDestroyLine extends BlockEventDestroyStandard
{
	private static float Duration = 0.5f;
	
	static 
	{
		BlockEventDestroyLine.Duration = FxSettingsRepository.getInstance().getSetting("BlockEventDestroyLine.Duration");
	}
	
	public BlockEventDestroyLine(boolean activate, BlockRenderable block)
	{
		this(BlockEventDestroyLine.Duration, activate, block);
	}

	public BlockEventDestroyLine(float time, boolean activate, BlockRenderable block)
	{
		super(time, activate, block);
		this.block = block;

		ParticleEffect effect = new ParticleEffect();
		effect.load(Gdx.files.internal("graphics/particles/line_erase.ps"), Gdx.files.internal("graphics/particles/"));
